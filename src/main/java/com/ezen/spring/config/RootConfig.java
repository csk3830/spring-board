...
public class RootConfig {
	...
	public DataSource dataSource() {
		...
		hikariConfig.setConnectionInitSql("SELECT now()");	// 첫 연결시 test sql
		...
		// connection당 캐싱할 preparedStatement 개수 지정 옵션 : default 250
		hikariConfig.addDataSourceProperty("dataSource.prepStmtsCacheSqlLimit", "true");	// 기본값 설정
		// mysql 서버에서 최신 이슈가 있을 경우 지원을 받을 것인지 설정
		hikariConfig.addDataSourceProperty("dataSource.useServerPrepStmts", "true");	// 기본값 설정
