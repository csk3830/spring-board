...
import com.ezen.spring.security.LoginFailHandler;
...
public class SecurityConfig extends WebSecurityConfigurerAdapter{
	...
	// 비밀번호를 암호화 하는 객체 PasswordEncoder에 대한 Bean 생성
	@Bean
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		.antMatchers("/","/user/login","/user/register", "/board/list", "/board/detail", "/upload/**", "/comment/**", "/resources/**") // 로그인 안해도 볼 수 있음.
		.permitAll()
		.anyRequest() // 위에 안쓴 모든 요청은
		.authenticated(); // 인증된 사용자만 접근 가능(그 외는 다 에러뜸)
